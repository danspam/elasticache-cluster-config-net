...
namespace Amazon.ElastiCacheCluster
{
    ...
    public class DiscoveryNode
    {
        ...
        internal static readonly int DefaultTryCount = 5;
        internal static readonly int DefaultTryDelay = 1000;
        ...
        internal DiscoveryNode(ElastiCacheClusterConfig config, string hostname, int port)
            : this(config, hostname, port, DefaultTryCount, DefaultTryDelay) { }
        ...
        internal DiscoveryNode(ElastiCacheClusterConfig config, string hostname, int port, int tries, int delay)
        {
            ...
            {
                throw new ArgumentException("The hostname is not able to use Auto Discovery (does not contain .cfg)");
            }
        ...
        {
            _config.Pool.UpdateLocator(new List<IPEndPoint>(new[] { _endPoint }));
        ...
        internal string GetNodeConfig()
        {
            ...
            var command = nodeVersion.CompareTo(older) < 0
                ? new GetOperation("AmazonElastiCache:cluster")
                : (IGetOperation) new ConfigGetOperation("cluster");
        ...
        internal IPEndPoint ResolveEndPoint()
        {
            ...
            while (tryCount > 0 && waiting)
            {
                ...
                {
                    tryCount--;
                    entry = Dns.GetHostEntryAsync(_hostname).Result;
            ...
            Log.DebugFormat("Resolved configuration endpoint {0} to {1}.", _hostname, entry.AddressList[0]);
